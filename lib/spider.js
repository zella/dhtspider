...
//Default was 2
var WALK_SPEED = process.env.WALK_SPEED_MILLIS || 20;
var JOIN_SPEED = process.env.JOIN_SPEED_MILLIS || 6000;
var TABLE_SIZE = process.env.TABLE_SIZE_DEFAULT || 600;
...
class Spider extends Emiter {
    constructor() {
        ...
        this.table = new Table(options.tableCaption || TABLE_SIZE)
    ...
    walk() {
        ...
        setTimeout(() => this.walk(), WALK_SPEED)
    }
    ...
    onGetPeersRequest(message, address) {
        ...
        this.emit('unensureHash', infohash.toString('hex').toLowerCase(), address)
    }
    ...
    onAnnouncePeerRequest(message, address) {
        ...
        this.emit('ensureHash', infohash.toString('hex').toLowerCase(), {
    ...
    listen(port) {
        ...
        setInterval(() => this.join(), JOIN_SPEED)
        this.join()
        this.walk()
    }
}
